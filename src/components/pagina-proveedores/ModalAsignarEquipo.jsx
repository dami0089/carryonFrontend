...
const ModalAsignarEquipo = () => {
  ...
  const { asignarEquipo, viajeObtenido } = useServicios();
  ...
  const handleSubmit = async (e) => {
    ...
    if (idEquipoProveedor === "no" || equiposData.length == 0) {
      ...
      const equipoSeleccionado = equiposData.find(
        (equipo) => equipo._id === idEquipoProveedor
      );

      if (equipoSeleccionado) {
        const chofer = equipoSeleccionado.chofer;
        const patenteCamion = equipoSeleccionado.camion;
        const patenteSemi = equipoSeleccionado.semis;
        await asignarEquipo(
          viajeObtenido._id,
          chofer,
          patenteCamion,
  ...
  return (
    <Transition.Root show={modalAsignarEquipo} as={Fragment}>
      <Dialog
        as="div"
        className="fixed inset-0 z-50 overflow-y-auto"
        onClose={handleAsignarEquipo}
      >
                    ...
                    {equiposData.length != 0 ? (
                      <div className="mb-1">
                        ...
                        <select
                          ...
                          onChange={(e) => setIdEquipoProveedor(e.target.value)}
                        >
