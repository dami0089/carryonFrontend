...
const ModalModificarEstadoServicio = () => {
  const {
  ...
  useEffect(() => {
    const estado = async () => {
      if (coincidoEstado) {
        setCambiarEstado(estadoObtenerServicio);
        setCoincidoEstado(false);
      }
    };
    estado();
  }, [coincidoEstado]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (idProveedorEditarEstadoServicio) {
      if (cambiarEstado === "Coordinado") {
        Swal.fire({
          title: "Queres notificar al cliente?",
          text: "Se enviara la informacion del viaje por mail",
          icon: "question",
          ...
          if (result.isConfirmed) {
            await cambiarEstadoServicio(idObtenerServicio, cambiarEstado);
      ...
      } else if (cambiarEstado === "Terminado") {
        Swal.fire({
          title: "Seguro queres terminar el vieje?",
          icon: "question",
          showCancelButton: true,
          cancelButtonText: "No",
          confirmButtonColor: "#3085d6",
      ...
      toast("⚠️ Para cambiar de estado, primero asigna un proveedor", {
        position: "top-right",
        autoClose: 1500,
        hideProgressBar: false,
        closeOnClick: true,
  ...
  return (
    <Transition.Root show={modalModificarEstadoServicio} as={Fragment}>
      <Dialog
        as="div"
        className="fixed inset-0 z-50 overflow-y-auto"
        onClose={handleModalModificarEstadoServicio}
      >
            ...
            <div className="inline-block transform overflow-hidden rounded-lg bg-white px-4 pt-5 pb-4 text-left align-bottom shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6 sm:align-middle">
              <div className="absolute top-0 right-0 hidden pt-4 pr-4 sm:block">
                <button
                  ...
                  onClick={handleModalModificarEstadoServicio}
                >
                  ...
                  <form className="my-2 mx-2" onSubmit={handleSubmit}>
                    <div className="mb-1">
                      ...
                      <select
                        ...
                        value={cambiarEstado}
                        onChange={(e) => setCambiarEstado(e.target.value)}
                      >
                        <option value="">--Seleccionar--</option>

                        {estadosServicioObtenidos.map((estado) => (
...
export default ModalModificarEstadoServicio;
